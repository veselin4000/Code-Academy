...
int timeInSeconds( tagTMyTime *time){
  return time->seconds+time->minutes*60+time->hours*60*60;
}

void secondsInTime( tagTMyTime *time,int sec){
  time->hours=sec/3600;
  time->minutes=(sec - (time->hours * 3600)) / 60;
  time->seconds=sec%60;
}

void addSeconds( tagTMyTime *time, int s){
  time->seconds+=s;
  if(time->seconds>=60){
    time->minutes+=1;
    time->seconds-=60;
...
void addMinutes( tagTMyTime *time, int m){
 ...
 tagTMyTime addHours( tagTMyTime *time, int h){
  time->hours+=h;
  if(time->hours>=24){
